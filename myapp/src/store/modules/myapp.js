...
export default {
  ...
  actions: {
     ...
     *ユーザー権限変更
     */
    async [ACTIONS.MYAPP_USER_KENGEN] ({ state, commit }, request) {
      const myAction = ACTIONS.MYAPP_USER_KENGEN
      ...
      // 実行
      await NHA_O_0116_API.kengenhenko(request)
        .then(function (response) {
          commit(MUTATIONS.MYAPP_USER_KENGEN_OK, response.data)
        })
        .catch(error => util.api.error(myAction, state.sync, error))
        .finally(() => util.api.end(myAction, state.sync))
    },
  ...
  mutations: {
    ...
    [MUTATIONS.MYAPP_USER_KENGEN] (state, data) {
      state.kengenhenkoItems = cloneDeep(data)
    },
